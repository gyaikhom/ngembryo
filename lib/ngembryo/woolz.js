/**
 * Encapsulates an IIP manager (based on Woolz IIP Viewer).
 * 
...
var WlzIIPViewer = new Class(
    {
        ...
        refresh: function() {
            this.model.setVals([ 'scl' ], [ this.model.scl.cur
            ], true, 'mouse');
        },
...
var Model = new Class(
    {
                ...
                switch (types[i]) {
                    ...
                    case 'scl':
                        ...
                        if (scl < this.scl.min)
                            ...
                            scl = this.scl.max;
                        scl = Math.pow(2,
                            Math.round(Math.log(scl) / Math.log(2)));
                        if (scl === this.scl.cur)
                            break;
                        this.scl.cur = scl;
        ...
        setWinMinScl: function() {
            ...
            if (this.wlz) {
                this.scl.winmin = Math.pow(2,
                    Math.floor(Math.log(min) / Math.log(2)));
                this.scl.initial = this.scl.winmin;
            } else {
                this.scl.winmin = 0;
                this.scl.initial = Math.pow(2,
                    Math.floor(Math.log(min) / Math.log(2)));
            }
            this.setMinScl();
        },
...
var TileFrame = new Class(
    {
        ...
        requestImages: function() {
            ...
            this.startx = Math.floor(this.model.roi2vpLeft() / this.model.tileSize.width)
                - hiddenBorder;
            this.starty = Math.floor(this.model.roi2vpTop() / this.model.tileSize.height)
                - hiddenBorder;
            this.endx = Math
                .floor((this.model.roi2vpLeft() + this.model.viewable.width) / this.model.tileSize.width)
                + hiddenBorder;
            this.endy = Math
                .floor((this.model.roi2vpTop() + this.model.viewable.height) / this.model.tileSize.height)
                + hiddenBorder;
            var refresh = false;
    ...
    var VarWin = new Class({
        initialize: function(source, model) {
            ...
            model.attach(this, [ typeEnum.viewport, typeEnum.scroll,
                typeEnum.scl,
                typeEnum.dst, typeEnum.dstquiet, typeEnum.pit,
                typeEnum.yaw,
                typeEnum.rol ]);
        ...
        update: function(types) {
            ...
            this.window.win.innerHTML = "<table width='200px'>"
                ...
                + "</td></tr>"
                + "<tr><td>yaw:</td><td>"
                + Math.round(this.model.yaw.cur * foo) / foo
                + "</td><td>"
                + Math.round(this.model.yaw.min * foo) / foo
                + "</td><td>"
                + Math.round(this.model.yaw.max * foo) / foo
                + "</td></tr>"
                + "<tr><td>rol:</td><td>"
                + Math.round(this.model.rol.cur * foo) / foo
                + "</td><td>"
                + Math.round(this.model.rol.min * foo) / foo
                + "</td><td>"
                + Math.round(this.model.rol.max * foo) / foo
                + "</td></tr>"
                + "</table>"
                + "<p>fxp: "
                + Math.round(this.model.fxp.x * foo) / foo
                + ","
                + Math.round(this.model.fxp.y * foo) / foo
                + ","
    ...
    var SectionPlane = new Class({
        ...
        convertPatchYawToTile: function(pitch, yaw) {
            var gap = 5, nCols = 72, nRows = 36, row = Math.floor(pitch / gap);
            row = (row === nRows) ? 0 : row; // to handle the pitch = 180deg case
            var col = Math.floor(yaw / gap);
            col = (col === nCols) ? 0 : col; // to handle the yaw = 360deg case
            /*
        ...
        update: function(types, trigger) {
            if (!this.model.wlz)
                return;
            this.window.setVisible(true);
            var k = this.convertPatchYawToTile(this.model.pit.cur, this.model.yaw.cur);
