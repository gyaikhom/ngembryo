...
var ControlManager = new Class( {
	...
	initialize : function(controls) {
		...
		this.connections.push(dojo.connect(tcv, "onclick", function() {
	...
	startup : function() {
		...
		if (this.controls.zoom) {
			...
			console.info("max, min, cur, r1, r2, tick:" + sclMax + "," + sclMin
					+ "," + sclCur + "," + rangeLow + "," + rangeHigh + ","
					+ curTick);
			$('zoomValue').set('value', sclCur);
			ngembryo.zoomSlider = new Slider($('zoomSliderControl'),
					$('zoomSlider'), {
						...
						onChange : function(step) {
							...
							if ($defined(woolz) && $defined(woolz.model)) {
								woolz.model.setVals( [ 'scl' ], [ scl ], true,
										'mouse');
							}
						},
						onComplete : function(step) {
							...
							$('zoomValue').set('value', scl);
							if ($defined(woolz) && $defined(woolz.model)) {
								woolz.model.setVals( [ 'scl' ], [ scl ], false,
										'mouse');
							}
						}
					}).set(curTick);
			this.connections.push(dojo.connect(dojo.byId("zoomValue"),
			...
			this.connections.push(dojo.connect(
					dojo.byId("zoomSliderDecrement"), "onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(
					dojo.byId("zoomSliderIncrement"), "onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
							var n = ngembryo.zoomSlider.step + 1;
							var h = ngembryo.zoomSlider.range[1];
							if (n > h)
		...
		if (this.controls.dst) {
			...
			ngembryo.dstSlider = new Slider($('dstSliderControl'),
					$('dstSlider'), {
						range : [ dstMin, dstMax ],
						snap : true,
						steps : dstMax - dstMin,
						...
						onChange : function(step) {
							...
							if ($defined(woolz) && $defined(woolz.model))
								woolz.model.setVals( [ 'dst' ], [ step ], true,
										'dstSlider');
						},
						onComplete : function(step) {
							if ($defined(woolz) && $defined(woolz.model))
								woolz.model.setVals( [ 'dst' ], [ step ],
										false, 'dstSlider');
						}
					}).set(dstCur);
			this.connections.push(dojo.connect(dojo.byId("dstValue"),
					"onchange", function(value) {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(dojo.byId("dstSliderDecrement"),
					"onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(dojo.byId("dstSliderIncrement"),
					"onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
							var n = ngembryo.dstSlider.step + 1;
							var h = ngembryo.dstSlider.range[1];
		...
		if (this.controls.navigator) {
			this.connections.push(dojo.connect(dojo.byId("navigatorLeft"),
					"onclick", function() {
						if ($defined(woolz.model) && !woolz.model.xfit) {
							...
							woolz.model.setVals( [ 'x' ], [ x ], false,
									'locator');
						}
					}));
			this.connections.push(dojo.connect(dojo.byId("navigatorRight"),
					"onclick", function() {
						if ($defined(woolz.model) && !woolz.model.xfit) {
							var x = 0.5;
							var t = woolz.locator.zone.offsetLeft + 15;
			...
			this.connections.push(dojo.connect(dojo.byId("navigatorUp"),
					"onclick", function() {
						if ($defined(woolz.model) && !woolz.model.yfit) {
							...
							woolz.model.setVals( [ 'y' ], [ y ], false,
									'locator');
						}
					}));
			this.connections.push(dojo.connect(dojo.byId("navigatorDown"),
					"onclick", function() {
						if ($defined(woolz.model) && !woolz.model.yfit) {
							var y = 0.5;
							var t = woolz.locator.zone.offsetTop + 15;
		...
		if (this.controls.roi) {
			ngembryo.pitchSlider = new Slider($('pitchSliderControl'),
					$('pitchSlider'), {
						range : [ 0, 179 ],
						snaps : true,
						steps : 180,
			...
			this.connections.push(dojo.connect(dojo.byId("pitchValue"),
					"onchange", function(value) {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(dojo
					.byId("pitchSliderDecrement"), "onclick", function() {
				if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(dojo
					.byId("pitchSliderIncrement"), "onclick", function() {
				if ($defined(woolz) && $defined(woolz.model)) {
					var n = ngembryo.pitchSlider.step + 1;
					var h = ngembryo.pitchSlider.range[1];
					if (n > h)
			...
			ngembryo.yawSlider = new Slider($('yawSliderControl'),
					$('yawSlider'), {
						range : [ 0, 359 ],
						snaps : true,
						steps : 360,
			...
			this.connections.push(dojo.connect(dojo.byId("yawValue"),
					"onchange", function(value) {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(dojo.byId("yawSliderDecrement"),
					"onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(dojo.byId("yawSliderIncrement"),
					"onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
							var n = ngembryo.yawSlider.step + 1;
							var h = ngembryo.yawSlider.range[1];
			...
			ngembryo.rollSlider = new Slider($('rollSliderControl'),
					$('rollSlider'), {
						range : [ 0, 359 ],
						snaps : true,
						steps : 360,
			...
			this.connections.push(dojo.connect(dojo.byId("rollValue"),
					"onchange", function(value) {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(
					dojo.byId("rollSliderDecrement"), "onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
			...
			this.connections.push(dojo.connect(
					dojo.byId("rollSliderIncrement"), "onclick", function() {
						if ($defined(woolz) && $defined(woolz.model)) {
							var n = ngembryo.rollSlider.step + 1;
							var h = ngembryo.rollSlider.range[1];
							if (n > h)
