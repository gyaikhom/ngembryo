 ...
 * 
 * Icons are used for displaying marker. Users are allowed to use their
 * own choice of icons, and the IconManager assist the user with the
 * management.
 */
var IconManager = Class({    
	annotationIcons: new Hash(), // All of the icons are stored in a hash.
	
    /**
     * This function registers an icon with the IconManager.
     *
     * @param {String} name Unique name of the icon (hash key).
     * @param {Object} icon Icon object to be registered.
     */
        ...
        if ($defined(this.annotationIcons)) {
            if (dojo.isString(name)) {
                if (dojo.isObject(icon)) {
					...
					console.info("Successfully registered icon: " + name);
                }
                else {
     ...
     * 
     * @param {Object} name Unique name of the icon (hash key).
     */
     ...
     * 
     * If an icon with the supplied name is not found,
     * the default icon object will be returned.
     *
     * @param {String} name Name of the icon object (hash key).
     * @return {Object} Icon object.
     */
