...
var Layer = new Class( {
	...
	refresh : function() {
		...
		dojo.xhrGet( {
			url : "getLayers.php?model=" + model + "&distance=" + dst + "&yaw="
					+ yaw + "&roll=" + rol + "&pitch=" + pit + "&format=json",
			handleAs : "json",
			sync : true,
	...
	create : function(flag) {
		...
		var url = "clay.php?model=" + model + "&distance=" + distance + "&yaw="
	...
	formatLayers : function() {
		...
		if ($defined(items)) {
			var content = "Click on the layer to select. Currently selected layer is highlighted.<br><br><div class='layer-browser'>"
					+ "<table class='items'>"
			...
			for ( var i = 0; i < items.length; i++) {
				...
				content += "<tr class='" + rowClass + ">"
						...
						+ "<td class='abstract'>" + items[i].description
						+ "</td></tr>";
			}
			content += "</table></div><br>" + buttons;
		} else {
			content = "No layers are currently registered with the system.<br><br>"
					+ buttons;
		}
		return content;
	},
