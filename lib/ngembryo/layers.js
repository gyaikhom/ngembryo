...
var Layer = new Class( {
	...
	refresh : function() {
		...
		dojo.xhrGet( {
			url : "gl.php?model=" + model + "&distance=" + dst + "&yaw=" + yaw
					+ "&roll=" + rol + "&pitch=" + pit,
			handleAs : "json",
			sync : true,
			timeout : 5000, /* Time in milliseconds. */
	...
	create : function(flag) {
		...
		var url = "clF.php?model=" + model + "&distance=" + distance + "&yaw="
		...
		if ($defined(items)) {
			...
			for ( var i = 0; i < items.length; i++) {
				...
				rowClass += selected ? " selected'" : " clickable' onClick='"
						+ "ngembryo.layer.selectCloseDialog(" + items[i].i
						+ ");'";
				...
				content += "<tr id='layer-identifier-" + items[i].i
						...
						+ ", this);'></td>" + "</td><td><button type='button'"
	...
	deleteLayer : function(e, lid, i) {
		confirmDialog("Delete layer", "Deleting a layer will also delete all "
			...
			if (flag) {
				...
				ngembryo.layer.__get("dl.php?lid=" + lid);
				if (lid == ngembryo.lid) {
