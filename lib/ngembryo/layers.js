...
var Layer = new Class(
		{
			...
			refresh : function() {
				if (ngembryo.oid != -1)
					ngembryo.orientation.old_oid = ngembryo.oid; // Use in
				// reset.
				ngembryo.oid = -1;
				...
				dojo
						.xhrGet( {
							url : "gl.php?model=" + model + "&distance="
									+ dst + "&yaw=" + yaw + "&roll=" + rol
									+ "&pitch=" + pit + "&format=json",
							handleAs : "json",
							sync : true,
							timeout : 5000, /* Time in milliseconds. */
							load : function(response, ioArgs) {
			...
			create : function(flag) {
				...
				var url = "clF.php?model=" + model + "&distance=" + distance
			...
			toggleLayerVisibility : function(e, index, elem) {
				...
				if (this.items[index].visible) {
					...
					this.select(this.items[index].id); /* Select this layer. */
			...
			setAllLayersVisibility : function(flag) {
				if ($defined(this.items)) {
					for ( var i = 0; i < this.items.length; i++) {
						...
						elem.checked = flag;
						var t = dojo.byId("layer-identifier-"
								+ this.items[i].id);
						if ($defined(t))
							dojo.removeClass(t, "selected");
					}
						...
						"Deleting a layer will also delete all "
							...
							if (flag) {
								dojo.byId("available-layers-table")
										.deleteRow(i);
								ngembryo.layer.__get("dl.php?lid="
										+ lid);
								if (lid == ngembryo.lid) {
				...
				if ($defined(items)) {
					...
					for ( var i = 0; i < items.length; i++) {
						...
						rowClass += selected ? " selected'"
								: " clickable' onClick='ngembryo.layer.selectCloseDialog("
										+ items[i].id + ");'";
						isVisible = (items[i].visible) ? "checked" : "";
						content += "<tr id='layer-identifier-"
								+ items[i].id
								+ "' class='"
								+ rowClass
								+ ">"
					...
					if (id == -1) {
						...
						if ($defined(this.items)) {
							...
							if (index != -1) {
								...
								ngembryo.content.feedback.show("info",
										"Layer '" + title + "' has been selected.");
							} else {
								ngembryo.content.feedback.show("warn",
										"Did not find layer with ID: "
												+ ngembryo.lid);
								return;
							}
						}
