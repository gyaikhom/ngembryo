...
var Orientation = new Class(
		{
			...
			refresh : function() {
				this.items = null;
				dojo.xhrGet( {
					url : "getOrientations.php?model=" + ngembryo.mid
							+ "&format=json",
					handleAs : "json",
					sync : true,
					timeout : 5000, /* Time in milliseconds. */
			 ...
			 * 
			 * @param flag
			 *            True if this is invoked from orientation browser
			 *            dialog.
			 */
			create : function(flag) {
				...
				var url = "cori.php?model=" + model + "&distance=" + distance
			...
			deleteOrientation : function(e, oid, i) {
				confirmDialog("Delete orientation",
						"Deleting an orientation will also "
							...
							if (flag) {
								dojo.byId("available-orientations-table")
										.deleteRow(i);
								ngembryo.orientation
										.__get("deleteOrientation.php?oid="
												+ oid);
								if (oid == ngembryo.oid)
									ngembryo.oid = -1;
			...
			formatOrientations : function() {
				...
				if ($defined(items) && items.length > 0) {
					/*
					 * This determines the size of the images displayed inside
					 * the orientation browser.
					 */
					var scale = 0.15;
					var navsrc = woolz.locator.server + '?' + woolz.model.fif
							+ "&mod=" + woolz.model.mode + "&fxp="
							+ woolz.model.fxp.x + ',' + woolz.model.fxp.y + ','
							+ woolz.model.fxp.z + "&scl=" + scale;
					...
					var content = "Click on the model to select. Currently selected model is highlighted.<br><br><div class='orientation-browser'>"
							...
							+ "<tr><th>Preview</th><th>Parameters</th><th>Action</th></tr>";
					for ( var i = 0; i < items.length; i++) {
						var rowClass = (i % 2) ? "oddRow" : "evenRow";
						var selected = (items[i].id == ngembryo.oid) ? true
								: false;
						rowClass += selected ? " selected'"
								: " clickable' onClick='ngembryo.orientation.select("
										+ items[i].id + ");'";
						var k = navsrc + "&dst=" + items[i].distance * scale
								+ "&pit=" + items[i].pitch + "&yaw="
								+ items[i].yaw + "&rol=" + items[i].roll
								+ "&qlt=" + woolz.model.qlt.cur + '&cvt=jpeg';
						...
						content += "<tr class='"
								...
								+ " onClick='ngembryo.orientation.deleteOrientation(event, "
								+ items[i].id + ", " + (i + 1)
								+ ");'>delete</button></td></tr>";
					}
					content += "</table></div><br>" + buttons;
				} else {
					content = "There are no registered orientations.<br>"
							+ buttons;
				}
				return content;
			},
			...
			select : function(id, flag) {
				if ($defined(this.items)) {
					...
					if (index != -1) {
						...
						ngembryo.controlManager
								.setPitchValue(this.items[i].pitch);
						ngembryo.controlManager
								.setRollValue(this.items[i].roll);
						...
						ngembryo.refresh();
						ngembryo.content.feedback.show("info",
								"Orientation '" + this.items[index].title + "' have been selected.");
					}
				}
			}
		});
