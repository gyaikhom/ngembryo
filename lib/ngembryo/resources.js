...
var Resource = new Class( {
	...
	edit : function(rid, e) {
		...
		erd = new dijit.Dialog( {
			...
			style : "width: 590px;",
			content : this.__getForm("ersF.php?rid=" + rid),
			onHide : function() {
				dojo.byId("resource-browser-dialog").focus();
			}
		});
	...
	addToAnnotation : function(rid, aid, type, n) {
		...
		dojo.xhrGet( {
			url : "arta.php?rid=" + rid + "&aid=" + aid + "&type=" + type,
			handleAs : "json",
			handle : function(data, args) {
				if (typeof data == "error") {
					console.warn("Error", args);
	...
	getAvailable : function(aid, type) {
		var url = "gar.php?aid=" + aid + "&type=" + type + "&exclude=1";
	...
	getDetails : function(id) {
		return this.__get("grs.php?rid=" + id);
	},
	...
	getLinked : function(aid, type) {
		var url = "gar.php?aid=" + aid + "&type=" + type + "&exclude=0";
		return this.__get(url);
	},
	...
	deleteAnnotation : function(e, aid, type) {
		confirmDialog("Delete annotation",
				"Are you sure you wish to delete this annotation?", function(
						flag) {
					if (flag) {
						ngembryo.resource.__get("da.php?aid=" + aid + "&type="
								+ type);
						ngembryo.engine.refresh();
	...
	unlink : function(e, aid, rid, type, n) {
		confirmDialog("Remove resource", "This will only unlink the resource"
			...
			if (flag) {
				...
				ngembryo.resource.__get("urfa.php?rid=" + rid + "&aid=" + aid
						+ "&type=" + type);
				ngembryo.engine.refresh();
				dojo.byId("display-linked-dialog").focus();
			}
	...
	editAnnotation : function(e, aid, type) {
		...
		ead = new dijit.Dialog( {
			...
			title : "Edit resource details",
			style : "width: 590px;",
			content : this.__getForm("eaF.php?aid=" + aid + "&type=" + type),
			onHide : function() {
				dojo.byId("display-linked-dialog").focus();
			}
		});
	...
	formatLinked : function(aid, type, label, description) {
		...
		var buttons = "<table width=100% border=0><tr>"
				+ "<td><button type='button'"
				+ " onClick='ngembryo.resource." + "editAnnotation(event, "
				+ aid + ", \"" + type + "\");'>Edit annotation"
				+ "</button> <button type='button'" + " onClick='ngembryo.resource."
				+ "linkToAnnotation(" + aid + ", \"" + type + "\");"
				+ "'>Add resource</button> " + "<button type='button'"
				+ " onClick='dijit.byId(\"display-"
				+ "linked-dialog\").hide();" + "'>Cancel</button></td>"
				...
				+ "\");'>Delete annotation"
				+ "</button></td></tr></table>";
		var content = "<b>Label:</b> " + label + "<br><b>Description:</b> "
				+ description;
		if ($defined(items) && items.length > 0) {
			content += "<br><br>The following resources"
					...
					+ "<tr><th>Author</th><th>Title"
					+ "</th><th>Abstract</th><th>Action</th></tr>";
			for ( var i = 0; i < items.length; i++) {
				var rowClass = (i % 2) ? "oddRow" : "evenRow";
				rowClass += " clickable' " + "onClick='window.open(\""
						+ items[i].l + "\");'";
