...
var Resource = new Class(
		{
			...
			formatAvailable : function(aid, type) {
				...
				if ($defined(items) && items.length > 0) {
					var content = "<table class='items' id='available-resources-table'>";
					...
					for ( var i = 0; i < items.length; i++) {
						...
						rowClass += " clickable' "
								...
								+ items[i].id + "," + aid + ", \"" + type
								+ "\"); dojo.byId(\"available-resources-table\").deleteRow(" + (i + 1) + ");'";
			...
			formatLinked : function(aid, type, label, description) {
				...
				if ($defined(items) && items.length > 0) {
					...
					content += "This annotation is not liked to any of the available resources.<br>"
							+ buttons;
				}
				return content;
			},
