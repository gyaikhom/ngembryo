...
var Resource = new Class(
		{
							...
							load : function(response, ioArgs) {
								if ($defined(response)) {
									if (response.success) {
										...
										if (!$defined(data)) {
											ngembryo.content.feedback.show(
			 ...
			 */
			getAll : function() {
				return this.__get("grs.php?format=json");
			},
			...
			create : function(flag) {
				...
				crd = new dijit.Dialog( {
					...
					style : "width: 590px;",
					content : this.__getForm("crsF.php"),
					onHide : function() {
						if (flag) {
			...
			addItem : function(rid) {
				...
				ari = new dijit.Dialog( {
					...
					title : "Create new resource",
					style : "width: 590px;",
					content : this.__getForm("criF.php?rid=" + rid),
					onHide : function() {
			...
			addToAnnotation : function(rid, aid, type, n) {
				/* Remove item as soon as it is added before starting server
				 * request. If the communication with the server is delayed,
				 * then it gives the impression that the item hasn't been
				...
				dojo.xhrGet( {
					url : "arta.php?rid=" + rid + "&aid=" + aid + "&type="
							+ type,
					handleAs : "json",
					handle : function(data, args) {
						if (typeof data == "error") {
			...
			getAvailable : function(aid, type) {
				var url = "gar.php?aid=" + aid + "&type=" + type
						+ "&exclude=1&format=json";
			...
			deleteResource : function(e, rid, i) {
				confirmDialog(
						"Delete resource",
						"Deleting a resource will also delete all of "
								+ "the associated resource items. "
								+ "Do you wish to continue?",
						function(flag) {
							if (flag) {
								ngembryo.resource.__get("dr.php?rid=" + rid);
								ngembryo.engine.refresh();

								/*
								 * If i > -1, this means that the delete command
								 * came from the resource listing dialog.
								 * Otherwise, the delete command came from
								 * resource details dialog.
								 */
								if (i > -1) {
									dojo.byId("available-resources-table")
											.deleteRow(i);
								} else {
							...
							/*
							 * If i > -1, this means that the delete command
							 * came from the resource listing dialog. Otherwise,
							 * the delete command came from resource details
							 * dialog.
							 */
							if (i > -1) {
								dojo.byId("resource-browser-dialog").focus();
			...
			getDetails : function(id) {
				return this.__get("grs.php?format=json&rid=" + id);
			},
			...
			deleteItem : function(rid, iid) {
				return this.__get("dri.php?rid=" + rid + "&iid=" + iid);
			},
			...
			deleteAnnotation : function(e, aid, type) {
				confirmDialog("Delete annotation",
						...
						function(flag) {
							if (flag) {
								ngembryo.resource.__get("da.php?aid=" + aid
										+ "&type=" + type);
								ngembryo.engine.refresh();
			...
			formatLinked : function(aid, type, label, description) {
				...
				var buttons = "<table width=100% border=0><tr>"
						...
						+ "'>Cancel</button></td>"
						+ "<td align=right><button type='button'"
						+ " onClick='ngembryo.resource.deleteAnnotation(event, "
						+ aid + ", \"" + type
						+ "\");'>Delete annotation</button></td></tr></table>";
				var content = "<b>Label:</b> " + label
						+ "<br><b>Description:</b> " + description;
				if ($defined(items) && items.length > 0) {
					content += "<br><br>The following resources are currently linked "
							+ "to this annotation.<br><div class='resource-browser'>"
							+ "<table class='items'>"
							+ "<tr><th>Author</th><th>Title</th><th>Abstract</th></tr>";
					for ( var i = 0; i < items.length; i++) {
						var rowClass = (i % 2) ? "oddRow" : "evenRow";
					...
					content += "<br><br>This annotation is not linked to any of the "
