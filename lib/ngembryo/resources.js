...
var Resource = new Class(
		{
				...
				if ($defined(items) && items.length > 0) {
					...
					if ($defined(ritems)) {
						content += "List of resource items:<br>"
								+ "<div class='resource-item-browser'><table class='items'>"
								+ "<tr><th>Title</th><th>Abstract</th><th>mime-type</th><th>Link</th></tr>";
			...
			formatLinked : function(aid, type, label, description) {
				...
				var content = "";
				if ($defined(items) && items.length > 0) {
					var content = "The following resources are currently linked to this annotation.<br><div class='resource-browser'>"
							+ "<table class='items'>";
					+"<tr><th>Author</th><th>Title</th><th>Abstract</th></tr>";
					for ( var i = 0; i < items.length; i++) {
						var rowClass = (i % 2) ? "oddRow" : "evenRow";
					...
					content += "This annotation is not liked to any of the available resources.<br><br>"
							+ buttons;
				}
				return content;
			},
