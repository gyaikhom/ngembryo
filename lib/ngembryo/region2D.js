        ...
        if ($defined(surface)) {
            ...
            var handleMouseEnter = function(event){
                if (!ngembryo.engine.isRegion2DLabelVisible()) {
                    ...
                    }, dijit.byId("content").domNode);
                    
                    dojo.create("div", {
                        id: "floatingLabel",
                        innerHTML: grpRegion.label
                    }, "floatingTipper");
                    
                    dojo.create("div", {
                        id: "floatingDescription",
                        innerHTML: grpRegion.description
                    }, "floatingTipper");
                }
            };
            ...
            this.eventConnections.push(dojo.connect(regionPolyline.getNode(), "onmousemove", function(event){
				...
				// These values, 300 and 165, come from ngembryo.css.
				if (x + 300 > ngembryo.engine.dim.w) left = true;
				if (y + 165 > ngembryo.engine.dim.h) up = true;
				
				// - 300 - 10, with horizontal adjustment for mouse pointer icon.
				// - 165 - 5, with vertical adjustment for mouse pointer icon.
				var l = left ? - 310 : 15;
				var t = up ? - 170 : 15;
                var ft = dojo.byId("floatingTipper");
                dojo.fx.slideTo({
                    node: ft,
                    left: x + l,
                    top: y + t,
            ...
            var handleMouseLeave = function(){
                if (!ngembryo.engine.isRegion2DLabelVisible()) {
                    regionPolyline.setFill([100, 0, 0, 0.25]);
                    dojo.destroy("floatingDescription");
                    dojo.destroy("floatingLabel");
                    dojo.destroy("floatingTipper");
                }
            };
