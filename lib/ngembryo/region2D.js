...
var Region2D = new Class({
    ...
    draw: function(surface, labelRenderer){
        if ($defined(surface)) {
            ...
            this.eventConnections.push(dojo.connect(regionPolyline.getNode(), "onmouseleave", function(){
                clearTimeout(timer);
            }));
            ...
            this.eventConnections.push(dojo.connect(regionPolylineH.getNode(), "onmouseleave", handleMouseLeave));
            this.eventConnections.push(dojo.connect(regionPolylineH.getNode(), "onclick", function(){
                ngembryo.dialogManager.showResources(grpRegion.id, grpRegion.label, grpRegion.description, grpRegion.type);
            }));
