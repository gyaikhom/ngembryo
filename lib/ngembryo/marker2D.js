...
var Marker2D = new Class( {
	...
	initialize : function(im, id, label, description, resources, x, y, icon) {
		...
		this.parent('2dmarker', id, label, description, resources);
	},
	 ...
	 * 
	 * @param {Object}
	 *            surface The Dojo surface to use as canvas.
	 */
		...
		if ($defined(surface)) {
			...
			if (dojo.isObject(i)) {
				...
				if ($defined(this.resources) && this.resources.count > 0) {
					...
					r = "This annotation is linked to <span style='color: #ff0000;'>" + this.resources.count + "</span> resources.<br><ul>";
				...
				var home = function(event) {
					...
					ngembryo.content.tipper.show(l, r);
				}
