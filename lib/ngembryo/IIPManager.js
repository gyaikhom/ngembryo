...
//			document.addEvent( 'click', this.hideContextMenu.bindWithEvent(this) );	
			...
			if (options.locator.enable)
				this.locator = new Locator( "roiControl", options.server, this.model);
...
var Model = new Class({
	...
	*/
	...
	getIIPMetadata: function() {
		...
		*/
		...
		if (ngembryo.controlReady == false)
	...
	*/
	...
	fixROI: function() {
		...
		if (this.xfit)
			this.roi.x = 0.5;
		else if (this.roi2vpLeft2() < 0)
			this.roi.x = this.vpl2roi(0);
		else if (this.roi2vpLeft2() + this.viewport.width > this.image.width)
			this.roi.x = this.vpl2roi(this.image.width - this.viewport.width);
		if (this.yfit)
			this.roi.y = 0.5;
		else if (this.roi2vpTop2() < 0)
			this.roi.y = this.vpt2roi(0);
		else if (this.roi2vpTop2() + this.viewport.height > this.image.height)
	...
	handleVariableChange: function(types, quiet, trigger) {
		...
		if ($defined(requiredObjects))
		{
	      ...
	      if (!quiet && ngembryo.controlReady && this.waiters == 0)
            ngembryo.refresh();
	},
	...
	setVals: function(types, values, quiet, trigger) {
		...
		for (var i = 0; i < types.length; i++)
		{
			if (!quiet) console.info("Changing " + types[i]);
			switch(types[i])
			{
				case 'dst':
					var dst = values[i];
	...
	vpt2roi: function(vp) {
		...
		return (vp + this.viewport.height/2) / this.image.height;
			...
			var url = this.server + "?" + this.fif+ "&mod=" + this.mode+ "&fxp=" + this.fxp.x + ',' + this.fxp.y + ','+ this.fxp.z + "&dst=" + this.dst.cur  + "&pit=" + this.pit.cur+ "&yaw=" + this.yaw.cur  + "&rol=" + this.rol.cur + "&scl=" + this.scl.cur +"&PRL=0," + x + "," + y + "&OBJ=Wlz-coordinate-3D";
			var response="";
			new Request( {
                isMetadata: true, //maze
						url: url,
						method: 'get',
						async: true,
						onSuccess: function(transport) {
...
var OverlayTileFrame = new Class({
	...
	setTileFramePosition: function() {
		...
		else
		{
			...
			this.overlayFrame.style.left = -this.model.roi2vpLeft()
		...
		else
		{
			...
			this.overlayFrame.style.top = -this.model.roi2vpTop()
	...
	requestImages: function() {
		...
		this.endx = Math.floor( (this.model.roi2vpLeft() + this.model.viewable.width)
				/ this.model.tileSize.width ) + hiddenBorder ;
		this.endy = Math.floor( (this.model.roi2vpTop() + this.model.viewable.height)
				/ this.model.tileSize.height ) + hiddenBorder ;
	...
	getDraggedTiles: function()
	{
		if (!this.model.xfit)
		{
			while (this.model.roi2vpLeft() + this.model.viewport.width
		...
		if (!this.model.yfit)
		{
			while (this.model.roi2vpTop() + this.model.viewport.height
...
this.maxHeight = dojo.style("roiControl", "height");
	...
	handleDrag: function(done) {
		...
		/**
		   Zone is the highlighted area inside the region of interest (Locator)
		   panel window. The value of this.zone.offsetLeft and this.zone.offsetTop
		   changes when the user drags the zone. Since we want to use navigator buttons,
		   we only need to update these varibales one of the four buttons are pressed.
		*/
		if (!this.model.xfit)
...
var SectionPlane = new Class({
	...
	initialize: function(source,server,model,options) {
		...
		this.image = new Element( 'img', {
			...
			'styles': {
				...
				'left': '0px',
											  'top': '0px',
											  'width': 128, // this.width,
											  'height': 128 //this.height
			}
		});
	...
	handleDrag: function(done) {
		...
		/* Since the dragging is in two dimensions, the value of roll cannot be set by dragging. */
		this.model.setVals(['pit','yaw'],[pit,yaw],!done,'sectionplane');
		
		/* Remember, we have to break the bi-directional event loop with the sliders. */
