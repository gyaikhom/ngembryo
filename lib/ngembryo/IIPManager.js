...
var WlzIIPViewer = new Class({
	...
	initialize: function(options) {
		...
		window.addEvent( 'domready', function() {
			...
			/**
			 * begin: Next Generation Embryology Project
			 *
			 * This initialises a wrapper for the NGEmbryo annotation engine.
			 * NOTE: The placement of the annotation engine call at this position is
			 * important, because this determines where the Dojo canvas is placed.
			 * At the moment, we only want the annotations to appear over the image data,
			 * but underneath the controls (if there are any, for instance zoom).
			 * @author gyaikhom
			 */
			if (options.ngembryo.enable)
			    new NGEmbryoWoolz(options.source, this.model, [typeEnum.ngembryo]);
			/**
			 * end: Next Generation Embryology Project
			 */

			if (options.locator.enable)
				new Locator( options.source, options.server, this.model,
						options.locator.maxSize[0], options.locator.maxSize[1])
					.window.setPosition( options.locator.pos[0], options.locator.pos[1]);

			if (options.sectionplane.enable)
				new SectionPlane(options.source, options.server, this.model,
...
var DraggableWindow = new Class({
	...
	initialize: function(source,title,model)
	{
		...
		this.left.injectInside(this.container);
		...
		this.right.injectInside(this.container);

		this.top = new Element('div', {
			'id': this.title + '-topedge',
			'class': 'edge',
			'styles': {
				'height': this.edgeWidth + 'px', 
				'left': this.edgeWidth + 'px',
				'top': '0px'
			}
		});
		this.top.injectInside(this.container);
		...
		this.bottom.injectInside(this.container);

		this.nwCorner = new Element('div', {
			'id': this.title + '-tlcorner',
			'class': 'corner corner-tl',
			'styles': {
				'width': this.edgeWidth + 'px',
				'height': this.edgeWidth + 'px',
				'background-image': "url(images/corners/NW10.gif)"
			}
		...
		new Element('div', {
			...
			'styles': {
				'width': this.edgeWidth + 'px',
				'height': this.edgeWidth + 'px',
				'background-image': "url(images/corners/NE10.gif)"
			}
		}).injectInside(this.container);

		new Element('div', {
			'id': this.title + '-blcorner',
			'class': 'corner corner-bl',
			'styles': {
				'width': this.edgeWidth + 'px',
				'height': this.edgeWidth + 'px',
				'background-image': "url(images/corners/SW10.gif)"
			}
		}).injectInside(this.container);

		new Element('div', {
			'id': this.title + '-brcorner',
			'class': 'corner corner-br',
			'styles': {
				'width': this.edgeWidth + 'px',
				'height': this.edgeWidth + 'px',
				'background-image': "url(images/corners/SE10.gif)"
			}
		}).injectInside(this.container);
		...
		/* this.handle.addEvent('click', function(event){
			if ( this.prevEventOver ){
				if ( !this.collapsed ) this.collapseWindow();
		...
		this.collapseButton.injectInside( this.handle );
		...
		this.container.makeDraggable({
			handle: this.title + '-handle',
			onComplete: function() {
				var x = (this.container.offsetLeft + this.width / 2 + this.edgeWidth < this.model.viewport.width / 2)
				? this.container.offsetLeft : this.container.offsetLeft - this.model.viewport.width + this.width + 2 * this.edgeWidth;
				var y = (this.container.offsetTop + this.height / 2 + this.edgeWidth < this.model.viewport.height / 2)
				? this.container.offsetTop : this.container.offsetTop - this.model.viewport.height + this.height + 2 * this.edgeWidth;
	...
	applyPosition: function() {
		...
		if (this.x >= 0)
			...
			this.container.style.left = this.model.viewport.width - this.width - 2 * this.edgeWidth + this.x + 'px';
		if (this.y >= 0)
			this.container.style.top = this.y + 'px';
		else if (this.y < 0)
			this.container.style.top = this.model.viewport.height - this.height - 2 * this.edgeWidth + this.y + 'px';
	},
	...
	setDimensions: function(wid,hei) {
		...
		this.container.style.width = Math.round(this.width + this.edgeWidth * 2) + 'px';
		this.container.style.height = Math.round(this.height + this.edgeWidth * 2) + 'px';
		this.handle.style.width = Math.round(this.width + this.edgeWidth * 2) + 'px';
		this.handle.style.height = Math.round(this.height + this.edgeWidth * 2) + 'px';
		this.left.style.height = Math.round(this.height) + 'px';
		this.right.style.height = Math.round(this.height) + 'px';
		this.top.style.width = Math.round(this.width) + 'px';
		this.bottom.style.width = Math.round(this.width) + 'px';
	...
	collapseWindow: function(){
		...
		this.expandWidth = this.top.getWidth();
		this.expandHeight = this.left.getHeight();
		this.collapseFrames = 20;
		...
		this.container.morph({height: collapseHeight+this.collapseFrames, width: collapseWidth+this.collapseFrames});
		this.win.morph({height:collapseHeight, width:collapseWidth}); //background morphs as well
		this.handle.morph({height: collapseHeight+this.collapseFrames, width: collapseWidth+this.collapseFrames});
		this.left.morph({height: collapseHeight});
		this.right.morph({height: collapseHeight});
		this.top.morph({width: collapseWidth});
		this.bottom.morph({width: collapseWidth});
	...
	expandWindow: function(){
		...
		this.container.morph({height:this.expandHeight+this.collapseFrames, width: this.expandWidth+this.collapseFrames});
		this.win.morph({height:this.expandHeight, width:this.expandWidth});
		this.handle.morph({height:this.expandHeight+this.collapseFrames, width: this.expandWidth+this.collapseFrames});
		this.left.morph({height:this.expandHeight});
		this.right.morph({height:this.expandHeight});
		this.top.morph({width:this.expandWidth});
...
var Locator = new Class({
	...
	initialize: function(source,server,model,maxwidth,maxheight) {
		...
		this.window = new DraggableWindow(source,title,model);
		this.window.setDimensions(maxwidth,maxheight);
		...
		this.imageContainer.injectInside( this.window.win );
	...
	update: function(types, trigger) {
		...
		if (types.indexOf(typeEnum.dst) != -1
				...
				//|| types.indexOf(typeEnum.pitquiet) != -1
				|| types.indexOf(typeEnum.yaw) != -1
				//|| types.indexOf(typeEnum.yawquiet) != -1
...
var SectionPlane = new Class({
	...
	initialize: function(source,server,model,options) {
		...
		this.width = this.options.tilesize[0];
		this.height = this.options.tilesize[1];
		this.textheight = 32;
		this.shorttitle = this.options.title.toLowerCase().split(" ").join("");

		this.window = new DraggableWindow(source,this.options.title,model);
		this.window.setDimensions(this.width,this.height + this.textheight);
		this.window.setPosition(this.options.pos[0], this.options.pos[1]);
	...
	update: function(types, trigger)
	{
		...
		this.pitchfb.innerHTML = "Pitch: "+this.model.pit.cur;
		this.yawfb.innerHTML = "Yaw: "+this.model.yaw.cur;
