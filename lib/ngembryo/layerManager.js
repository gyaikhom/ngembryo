 ...
 *
 * All of the annotations (2D and 3D markers, and 2D regions) are
 * managed as collections of annotations. These collections are defined
 * using layers. Annotations are associate with a layer, and these
 * layers are either displayed or hidden. Only layers that are
 * visible are rendered on the Dojo surface.
 */
var LayerManager = new Class({
     ...
     * 'name'is used to identify a layer. If 'visible' is undefined or
     * true, the new layer will be registered as visible. Otherwise, it
     * will be registered as invisible. This only applies to
     * initialisation, since the visibility of layers can be altered later.
     * The annotations are the collection of items that are currently
     * visible inside the region-of-interest marked by the surface bounding
     * box. The annotations could be undefined if there are no items
     * currently visible inside the region-of-interest.
     *
     * @param {String} name Unique name of the new layer (used as hash key).
     * @param {Boolean} visible Layer visibility (true if visible, otherwise false).
     */
     ...
     *
     * @param {String} name Unique name of the layer to be removed (hash key).
     */
     ...
     *
     * @param {String} name Unique name of the layer (hash key).
     * @param {Boolean} visibility Layer visibility (true if visible, otherwise false).
     */
     ...
     *
     * @param {String} name Layer to which the annotation will be attached (hash key).
     * @param {Annotation} annotation The annotation to be attached.
     */
     ...
     * @param {String} name Layer from which to remove the annotation (hash key).
     * @param {Integer} annotationID The unique identifier of the annotation.
     */
     ...
     *
     * @param {String} name Layer from which to remove all of the annotations (hash key).
     */
    emptyLayer: function(name){
        console.debug("Emptied layer '" + name + "'");
        if ($defined(this.annotationLayers)) {
            var layer = this.annotationLayers.get(name);
            if ($defined(layer)) {
                if ($defined(layer.annotationList)) {
                    ...
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        if ($defined(a)) {
                            a.destroy();
                            console.info("Successful destroyed.");
                        }
                        else {
                            console.debug("Annotation not defined.");
                        }
                    }
     ...
     *
     * @param {Object} surface The Dojo surface to be used as canvas.
     * @param {String} name Unique name of the layer (hash key).
     */
                    ...
                    if (layer.visible === true) {
                        if ($defined(layer.annotationList)) {
                            ...
                            for (var i = 0; i < keys.length; i++) {
                                var a = layer.annotationList.get(keys[i]);
                                if ($defined(a)) {
                                    a.draw(surface, this.labelRenderer);
                                    console.info("Successful draw.");
                                }
                                else {
                                    console.debug("Annotation not defined.");
                                }
                            ...
                            console.debug("Layer '" + name + "' is empty. No annotation found.");
                        }
                    }
                    else {
                        console.debug("Layer '" + name + "'is invisible.");
                    }
                }
                else {
     ...
     *
     * @param {Object} Dojo surface where the annotations are to be rendered.
     */
