...
var DialogManager = new Class({
     ...
     */
    setCreate2DMarkerDialog: function(x, y){
        dojo.xhrGet({
            url: "c2dm.php?x=" + x + "&y=" + y,
            handleAs: "text",
            timeout: 5000, // Time in milliseconds
            load: function(response, ioArgs){
                dialog.attr({
     ...
     */
    setCreate2DRegionDialog: function(polyline){
        var str = "";
        var i = 0;
    ...
    __parseResources: function(data){
        ...
        for (var i = 0; i < data.length; i++) {
            content += "<li>" + data[i].id + data[i].title + data[i].abstract + data[i].url + "</li>";
        }
        content += "</ul></div>";
        return content;
    },
     ...
     * @param {Object} y y-coordinate of the marker.
     */
    create2DMarker: function(x, y){
        this.setCreate2DMarkerDialog(x, y);
        dialog.show();
    },
     ...
     */
    create2DRegion: function(p){
        this.setCreate2DRegionDialog(p);
        dialog.show();
    }
