 ...
 * @param question
 *            The quetion to confirm.
 * @param callbackFn
 *            Callback function to call when confirmation.
 * @param onHideFn
 *            Function to call when the confirmation dialog hides.
 * @param e
 *            Event which resulted in the confirmation dialog.
 */
function confirmDialog(title, question, callbackFn, onHideFn, e) {
    ...
    var callback = function(mouseEvent) {
        ...
        if (mouseEvent.target)
            srcEl = mouseEvent.target; // Firefox.
        else {
            if (mouseEvent.srcElement)
                srcEl = mouseEvent.srcElement; // IE
        }
        if ($defined(srcEl)) {
            if (srcEl.nodeType === 3) // Fix Safari bug
                srcEl = srcEl.parentNode;

            /* Safari puts _label after the id that was specified
             * when the button is created. Because of this, using id
             * does not work on Safari. */
            if (srcEl.innerHTML === 'Yes' || srcEl.id === 'yes') {
                callbackFn(true);
            } else {
                callbackFn(false);
            }
