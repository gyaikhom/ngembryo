...
var AnnotationEngine = new Class({
    initialize: function(target){
        ...
        this.controlsOn = this.cookieManager.get("controlsOn", false);
        /* If true, controls are usable. */
        this.controlsActive = this.cookieManager.get("controlsActive", false);
    ...
    refresh: function(){
        if ($defined(this.surface)) {
            if (this.m2DV && $defined(ngembryo.layer)) {
                ...
                if ($defined(ngembryo.layer.items)) {
                    for (var i = 0; i < ngembryo.layer.items.length; i++) {
                        this.get2DMarkers(ngembryo.layer.items[i].id, 0, 0, 1900, 886, 0, 10);
                    }
                }
            }
            ...
            if (this.r2DV && $defined(ngembryo.layer)) {
                ...
                if ($defined(ngembryo.layer.items)) {
                    for (var i = 0; i < ngembryo.layer.items.length; i++) {
                        this.get2DRegions(ngembryo.layer.items[i].id, 0, 0, 1900, 886, 0, 10);
                    }
                }
            }
    ...
    createSurface: function(){
        ...
        if ($defined(this.parent)) {
            ...
            console.error("Failed to create Dojo surface: Target canvas '" +
            this.target +
    ...
    attachDraggingEvent: function(){
        ...
        var handleDragStart = function(event){
            ...
            ngembryo.engine.draggableDrag = dojo.connect(surfaceNode, "onmousemove", function(event){
            ...
            ngembryo.engine.draggableStop = dojo.connect(surfaceNode, "onmouseup", function(event){
                var dx = ngembryo.engine.dragStartX -
                event.clientX;
                var dy = ngembryo.engine.dragStartY -
                event.clientY;
        ...
        this.draggableStart = dojo.connect(surfaceNode, "onmousedown", handleDragStart);
    },
    ...
    attachScrollEvent: function(){
        //this.scrolling = dojo.connect(dojo.byId("tileframe"), (!dojo.isMozilla ? "onmousewheel" : "DOMMouseScroll"), function(e){
            this.scrolling = dojo.connect(ngembryo.engine.surface.getEventSource(), (!dojo.isMozilla ? "onmousewheel" : "DOMMouseScroll"), function(e){
    ...
    detachScrollEvent: function(){
        if ($defined(this.scrolling)) {
            //dojo.disconnect(dojo.byId("tileframe"), (!dojo.isMozilla ? "onmousewheel" : "DOMMouseScroll"));
            dojo.disconnect(ngembryo.engine.surface.getEventSource(), (!dojo.isMozilla ? "onmousewheel" : "DOMMouseScroll"));
        }
    },
    ...
    __update2DMarkers: function(layerManager, iconManager, data){
        ...
        var tfal = woolz.tileframe.tileFrameContainer.offsetLeft +
        woolz.tileframe.tileFrame.offsetLeft;
        var tfat = woolz.tileframe.tileFrameContainer.offsetTop +
        woolz.tileframe.tileFrame.offsetTop;
     ...
     *
     * @param {Integer} lid Unique ID of parent layer for the markers.
     * @param {Integer}
    ...
    get2DMarkers: function(lid, xl, yl, xh, yh, sl, sh){
        ...
        dojo.xhrGet({
            url: "get2DMarkers.php?lid=" + lid + "&x_low=" + xl + "&x_high=" + xh +
            "&y_low=" +
            yl +
            "&y_high=" +
            yh +
                ...
                if ($defined(response)) {
                    if (response.success) {
                        if ($defined(response.markers)) 
                            func(layerManager, iconManager, response.markers);
                    }
                    else {
     ...
     * of the Dojo surface.
     *
     * @param {Integer} lid Unique ID of parent layer for the regions.
     *
     * @param {Integer}
     *            xl x-coordinate of top-left corner.
     * @param {Integer}
    ...
    get2DRegions: function(lid, xl, yl, xh, yh, sl, sh){
        ...
        var layerManager = this.layerManager;
        dojo.xhrGet({
            url: "get2DRegions.php?lid=" + lid + "&x_low=" + xl + "&x_high=" + xh +
            "&y_low=" +
            yl +
            "&y_high=" +
            yh +
    ...
    createRegion: function(grpRegion, px, py, x, y){
        ...
        if (this.__isCloseToFinalPoint(px, py)) {
             ...
             */
            ngembryo.dialogManager.create2DRegion(this.tempRegionPoints, woolz.model.scl.cur, ngembryo.lid);
        ...
        if ($defined(this.surface)) {
            ...
            var handleMouseOverIcon = function(event){
                ...
                py < woolz.tileframe.tileFrameContainer.offsetHeight) {
                    ngembryo.dialogManager.create2DMarker(px, py, woolz.model.scl.cur, ngembryo.lid);
                }
            };
            this.eventCanvas.push(dojo.connect(cc.getNode(), "onclick", handleMouseOverIcon));
    ...
    activateCreateCanvas2DRegion: function(){
        if ($defined(this.surface)) {
            ...
            this.eventCanvas.push(dojo.connect(cc.getNode(), "onclick", handleCreateCanvasClick));
