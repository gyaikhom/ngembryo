/**
 * Encapsulates an annotation rendering engine.
 * 
...
var AnnotationEngine = new Class({
     ...
     * 
     * @param {String} url Where to retrieve the form from.
     */
    ...
    search: function() {
        ...
        sd = new dijit.Dialog({
            ...
            title: "Search",
            style: "width: 70%;",
            content: this.__getForm("searchForm.php")
        });
        ...
        if (this.mV && $defined(ngembryo.layer)) {
            ...
            if ($defined(ngembryo.layer.items)) {
                ...
                for (var i = 0; i < t.length; i++) {
                    if (t[i].v) {
                        this._GM(t[i].i, this.xl, this.yl, this.xh, this.yh);
                    }
                }
            }
        }
    ...
    _SR: function() {
        if (this.rV && $defined(ngembryo.layer)) {
            ...
            if ($defined(ngembryo.layer.items)) {
                ...
                for (var i = 0; i < t.length; i++) {
                    if (t[i].v) {
                        this._GR(t[i].i, this.xl, this.yl, this.xh, this.yh);
                    }
                }
            }
        }
    ...
    _CT: function(resources) {
        var a = "";
        var b = "";
        if ($defined(resources) && resources.c > 0) {
            var r = resources.r;
            a = "<ul>";
            for (var i = 0; i < r.length; i++) {
                a += "<li><span class='title'>" + r[i].t
                    + "</span> by <span class='author'>" + r[i].a
                    + "</span></li>";
            }
            a += "</ul>";
            b = "Annotation linked to <span style='color: #ff0000;'>"
                + resources.c + "</span> resources. Click for more...";
        } else {
    ...
    /**
     * Parse markers in a specific scale.
     * 
     * @param {Object}
     *            lm Layer manager.
     * @param {Object}
     *            im Icon manager.
     * @param {Object}
     *            data Array of marker objects.
     */
    __PM: function(lm, im, data) {
        for (var i = 0; i < data.length; i++) {
            /* Scaling adjustment. */
            data[i].x = (data[i].x * this.scl) / data[i].s;
            data[i].y = (data[i].y * this.scl) / data[i].s;
            ...
            var temp = new Marker(im, data[i].id, data[i].l, data[i].d,
                data[i].r, data[i].x, data[i].y, icon);
    ...
    __UM: function(lm, im, data) {
        this.__PM(lm, im, data[0]); /* Scale 1 */
        this.__PM(lm, im, data[1]); /* Scale 2 */
        this.__PM(lm, im, data[2]); /* Scale 4 */
    },
    /**
     ...
     */
    _GM: function(lid, xl, yl, xh, yh) {
        dojo.xhrGet({
            url: "getMarkersAndResources.php?lid=" + lid + "&x_low=" +
                xl + "&x_high=" + xh + "&y_low=" + yl + "&y_high=" +
                yh + "&scale=" + this.scl,
            handleAs: "json",
            timeout: 5000,
            sync: true,
            load: function(response) {
                if ($defined(response)) {
                    if (response.success) {
                        if ($defined(response.m)) {
                            try {
                                this.__UM(this.lm, this.im, response.m);
                            } catch (err) {
    ...
    /**
     * Parse regions in a specific scale.
     * 
     * @param {Object}
     *            lm Layer manager.
     * @param {Object}
     ...
     */
    __PR: function(lm, data) {
        for (var i = 0; i < data.length; i++) {
            var pl = data[i].p;
            for (var j = 0; j < pl.length; j++) {
                /* Scaling adjustment. */
                pl[j].x = (pl[j].x * this.scl) / data[i].s;
                pl[j].y = (pl[j].y * this.scl) / data[i].s;

                /* Tileframe adjustment. */
                pl[j].x += this.ax;
                pl[j].y += this.ay;
            }
            var temp = new Region(data[i].id, data[i].l, data[i].d, data[i].r,
                pl);
    ...
    __UR: function(lm, data) {
        this.__PR(lm, data[0]); /* Scale 1 */
        this.__PR(lm, data[1]); /* Scale 2 */
        this.__PR(lm, data[2]); /* Scale 4 */
    },
    /**
     ...
     *            yh y-coordinate of bottom-right corner.
     */
    _GR: function(lid, xl, yl, xh, yh) {
        dojo.xhrGet({
            url: "getRegionsAndResources.php?lid=" + lid + "&x_low=" +
                xl + "&x_high=" + xh + "&y_low=" + yl + "&y_high=" +
                yh + "&scale=" + this.scl,
            handleAs: "json",
            timeout: 5000, /* Time in milliseconds. */
