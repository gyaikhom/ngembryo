...
var AnnotationEngine = new Class( {
	...
	__RI : function(p) {
		...
		if ($defined(this.im)) {
			this.im.registerIcon('default', new Icon(p + 'bookmark.png', 16,
					16, 'cc'));
			this.im.registerIcon('bookmark', new Icon(p + 'Marker.png', 16, 16,
					'dd'));
	...
	__fSR : function(results, type) {
		...
		if (results.length < 1) {
			return "Search did not match any item.<br>"
		}
		content = "Click on the corresponding row select result.<br><br>"
				+ "<div class='search-result-browser'>"
				+ "<table class='items' id='search-result-table'>";

		switch (type) {
		case 1:
			content += "<tr><th>Author</th><th>Title"
			...
			for ( var i = 0; i < results.length; i++) {
				...
				content += "<td class='author'>" + results[i].a + "</td>";
				content += "<td class='title'>" + results[i].t + "</td>";
				content += "<td class='abstract'>" + results[i].d + "</td>";
				content += "</tr>";
			}
			break;
		case 2:
		case 3:
		case 4:
			content += "<tr><th>Title</th><th>Description</th></tr>";
			for ( var i = 0; i < results.length; i++) {
				var rowClass = (i % 2) ? "oddRow" : "evenRow";
				rowClass += " clickable' " + "onClick='ngembryo.engine.goTo("
						+ results[i].m + "," + results[i].o + ","
						+ results[i].l + "," + results[i].s + ");";
				content += "<tr class='" + rowClass + "'>";
				content += "<td class='title'>" + results[i].t + "</td>";
				content += "<td class='abstract'>" + results[i].d + "</td>";
				content += "</tr>";
	...
	goTo : function(m, o, l, s) {
		...
		var rd = dijit.byId("show-search-result-dialog");
		if ($defined(rd)) {
			dojo.body().removeChild(rd.domNode);
			rd.destroyRecursive(false);
		}
	...
	__dSR : function(results, type) {
		...
		rd = new dijit.Dialog( {
			id : "show-search-result-dialog",
			title : "Search results",
			style : "width: 590px;",
			content : this.__fSR(results, type),
			onHide : function() {
				dojo.byId("search-dialog").focus();
			}
		});
		dojo.body().appendChild(rd.domNode);
		rd.startup();
		rd.show();
	},
	...
	__GPSR : function(event) {
		dojo.xhrGet( {
			...
			load : function(response, ioArgs) {
				if ($defined(response)) {
					if (response.success) {
						this.__dSR(response.results, response.type);
					} else {
	...
	_GM : function(lid, xl, yl, xh, yh, sl, sh) {
		dojo.xhrGet( {
			url : "gm.php?lid=" + lid + "&x_low=" + xl + "&x_high=" + xh
					+ "&y_low=" + yl + "&y_high=" + yh + "&scale_low=" + sl
					+ "&scale_high=" + sh + "&format=json",
			handleAs : "json",
			timeout : 5000,
			sync : true,
