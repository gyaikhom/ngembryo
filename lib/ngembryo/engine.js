...
var AnnotationEngine = new Class(
		{
			...
			__update2DMarkers : function(layerManager, iconManager, data) {
				...
				for ( var i = 0; i < data.length; i++) {
					...
					var temp = new Marker2D(iconManager, data[i].id,
							data[i].label, data[i].description, px, py,
							"bookmark");
			...
			get2DMarkers : function(xl, yl, xh, yh, sl, sh) {
				...
				dojo.xhrGet( {
					url : "get2DMarkers.php?x_low=" + xl + "&x_high=" + xh
							+ "&y_low=" + yl + "&y_high=" + yh + "&scale_low="
							+ sl + "&scale_high=" + sh + "&format=json",
					handleAs : "json",
			...
			__update2DRegions : function(layerManager, data) {
				...
				for ( var i = 0; i < data.length; i++) {
					...
					var temp = new Region2D(data[i].id, data[i].label,
							data[i].description, data[i].tl_x, data[i].tl_y,
			...
			get2DRegions : function(xl, yl, xh, yh, sl, sh) {
				...
				var layerManager = this.layerManager;
				dojo.xhrGet( {
					url : "get2DRegions.php?x_low=" + xl + "&x_high=" + xh
							+ "&y_low=" + yl + "&y_high=" + yh + "&scale_low="
							+ sl + "&scale_high=" + sh + "&format=json",
					handleAs : "json",
					timeout : 5000, /* Time in milliseconds. */
			...
			createRegion : function(grpRegion, px, py, x, y) {
				...
				if (this.__isCloseToFinalPoint(px, py)) {
					...
					ngembryo.dialogManager.create2DRegion(
							...
							woolz.model.dst.cur, woolz.model.yaw.cur,
							woolz.model.rol.cur, woolz.model.pit.cur);
					...
					var handleMouseOverIcon = function(event) {
						...
						if (px >= 0
							...
							ngembryo.dialogManager.create2DMarker(px, py,
									woolz.model.scl.cur, woolz.model.dst.cur,
									woolz.model.yaw.cur, woolz.model.rol.cur,
									woolz.model.pit.cur);
						}
					};
