...
var AnnotationEngine = new Class({
    initialize: function(target){
        ...
        this.cookieManager = new CookieManager(); // Initialise cookie manager.
        /**
         * TODO: Could put all of this as a JSON string. Then we will have to
         * call dojo.cookie() only once.
         */
        this.controlsOn = this.cookieManager.get("controlsOn", true); // If true, displays controls.
        this.m2DV = this.cookieManager.get("m2dv", true); // If true, displays 2D markers.
        this.m3DV = this.cookieManager.get("m3dv", true); // If true, displays 3D markers.
        this.r2DV = this.cookieManager.get("r2dv", true); // If true, displays 2D regions.
        ...
        this.ccHandler = null; // Handles annotation creation on the canvas.
        this.eventCanvas = []; // List of event handlers for the canvas.
        this.eventLastPoint = []; // Event handler for polyline completion end point.
        ...
        this.tempRegionPoints = new Array(); // Holds the polyline coordinates relative to tileframe.
        this.circumference = 0; // TODO: Circumference of polygon.
        this.actual3Dcoord = null; // TODO: Actual 3D coordinate of points.
    },
    ...
    start: function(){
        if ($defined(this.target)) {
            ...
            this.labelRenderer = new LabelRenderer("Arial", "normal", "bold", "9pt", "black", null, "#FFF0B8", null, 0, 10, 50, 50, true);
            ...
            this.iconManager.registerIcon('bookmark', new Icon('resources/images/Marker2D.png', 16, 16, 'dd'));
    ...
    __update2DMarkers: function(layerManager, iconManager, data){
        ...
        /* Values for tileframe adjustment. */
        var tfal = woolz.tileframe.tileFrameContainer.offsetLeft + woolz.tileframe.tileFrame.offsetLeft;
        var tfat = woolz.tileframe.tileFrameContainer.offsetTop + woolz.tileframe.tileFrame.offsetTop;
        for (var i = 0; i < data.length; i++) {
            /* Scaling adjustment. */
			var px = (data[i].x * woolz.model.scl.cur) / data[i].scale;
			var py = (data[i].y * woolz.model.scl.cur) / data[i].scale;
            px += tfal;
            py += tfat;
            var temp = new Marker2D(iconManager, data[i].id, data[i].label, data[i].description, px, py, "bookmark");
    ...
    __update2DRegions: function(layerManager, data){
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < pl.length; j++) {
                /* Scaling adjustment. */
                pl[j].x = (pl[j].x * woolz.model.scl.cur) / data[i].scale;
                pl[j].y = (pl[j].y * woolz.model.scl.cur) / data[i].scale;
                pl[j].x += tfal;
                pl[j].y += tfat;
            }
            var temp = new Region2D(data[i].id, data[i].label, data[i].description, data[i].tl_x, data[i].tl_y, data[i].br_x, data[i].br_y, pl);
     ...
     * If this returns true, the polyline will be closed to form a polygon.
     *
    ...
    createRegion: function(grpRegion, px, py, x, y){
         ...
         * functionalities (measurement, polyline creation etc.)
         * are carried out using screen coordinates.
         */
        var t = {
            x: px,
            y: py
        ...
        /* This is the coordinate relative to the tileframe
         * where the model has been displayed. All annotations
         * are created using this coordinate.
         */
        var q = {
            x: x,
            y: y
        ...
        if (this.__isCloseToFinalPoint(px, py)) {
            ...
            ngembryo.dialogManager.create2DRegion(this.tempRegionPoints, woolz.model.scl.cur, woolz.model.dst.cur, woolz.model.yaw.cur, woolz.model.rol.cur, woolz.model.pit.cur);
    ...
    /**
     * Activates the create canvas (for 2D marker annotation) overlay over the target canvas.
     *
    ...
    activateCreateCanvas2DMarker: function(){
        if ($defined(this.surface)) {
            ...
            this.eventCanvas.push(dojo.connect(cc.getNode(), "onclick", function(event){
                ...
                /* Only act on this position if it falls inside the tileframe, where the
                 * images of the models are displayed.
                 */
                if (px >= 0 && py >= 0 && px < woolz.tileframe.tileFrameContainer.offsetWidth && py < woolz.tileframe.tileFrameContainer.offsetHeight) {
                    ngembryo.dialogManager.create2DMarker(px, py, woolz.model.scl.cur, woolz.model.dst.cur, woolz.model.yaw.cur, woolz.model.rol.cur, woolz.model.pit.cur);
                }
            }));
    ...
    /**
     * Activates the create canvas (for 2D region annotation) overlay over the target canvas.
     *
     * Existing markers and regions that are displayed on the target canvas will be inactive
     * but visible through a translucent veil. This helps the user to see what is already
     * marked so that clutter is reduced.
    ...
    activateCreateCanvas2DRegion: function(){
        if ($defined(this.surface)) {
            ...
            this.eventCanvas.push(dojo.connect(cc.getNode(), "onclick", function(event){
                 ...
                 */
                if (px >= 0 && py >= 0 && px < woolz.tileframe.tileFrameContainer.offsetWidth && py < woolz.tileframe.tileFrameContainer.offsetHeight) {
                    var temp = ngembryo.engine.createRegion(grpRegion, x, y, px, py);
                }
            }));
            this.eventCanvas.push(dojo.connect(cc.getNode(), "onmousemove", function(event){
                if (ngembryo.engine.regionPoints.length > 0) {
                    ...
                    if (ngembryo.engine.__isCloseToFinalPoint(px, py)) {
                    ...
                    if (ngembryo.engine.regionPoints.length > 0) {
                        dojo.forEach(ngembryo.engine.eventLastPoint, dojo.disconnect);
                    ...
                    circ.setFill([0, 0, 255, 1]);
                    circ.setStroke("black");
                    ngembryo.engine.eventLastPoint.push(dojo.connect(circ.getNode(), "onclick", function(event){
                         ...
                         */
                        ngembryo.engine.createRegion(grpRegion, ngembryo.engine.regionPoints[0].x, ngembryo.engine.regionPoints[0].y, ngembryo.engine.tempRegionPoints[0].x, ngembryo.engine.tempRegionPoints[0].y);
                        dojo.forEach(ngembryo.engine.eventLastPoint, dojo.disconnect);
                    }));
                    ngembryo.engine.eventLastPoint.push(dojo.connect(circ.getNode(), "onmouseenter", function(event){
                        circ.setFill([255, 255, 255, 1]);
                    }));
                    ngembryo.engine.eventLastPoint.push(dojo.connect(circ.getNode(), "onmouseleave", function(event){
                        circ.setFill([0, 0, 255, 1]);
                    }));
                }
            }));
    ...
    /**
     * TODO: Finds the area of a 3D planar polygon.
     * Algorithm from:
    ...
    __findAreaOfPolygon: function(){
        ...
        for (var i = 0, j = 0; i < this.regionPoints.length; i++) {
            ...
            if (j == this.regionPoints.length) 
                j = 0;
            area += (this.regionPoints[i].x + this.regionPoints[j].x) * (this.regionPoints[i].y - this.regionPoints[j].y);
        }
        
        /* We return the absolute value because the
         * area of the polygon is positive when the polygon
         * is counter-clockwise; negative if clockwise.
         * The area remains the same (except for the sign).
         */
    ...
    measure: function(grpRegion, px, py, x, y){
        ...
        /* Get the 3D woolz coordinate of this point.
         * TODO: This will be used to calsulate the actual coordinates
         * in 3D, and then to calculate the area dn circumference in 3D.
         */
    ...
    activateMeasurementCanvas: function(){
        if ($defined(this.surface)) {
            ...
            this.eventCanvas.push(dojo.connect(cc.getNode(), "onclick", function(event){
                 ...
                 */
                if (px >= 0 && py >= 0 && px < woolz.tileframe.tileFrameContainer.offsetWidth && py < woolz.tileframe.tileFrameContainer.offsetHeight) {
                    ngembryo.engine.measure(grpRegion, x, y, px, py);
                }
            }));
            this.eventCanvas.push(dojo.connect(cc.getNode(), "onmousemove", function(event){
                if (ngembryo.engine.regionPoints.length > 0) {
                    ...
                    ngembryo.engine.__drawPolylineWithMeasurements(grpRegion, false);
                    ngembryo.engine.regionPoints.pop();
                    var circ = grpRegion.createCircle({
                        cx: ngembryo.engine.regionPoints[0].x,
                        cy: ngembryo.engine.regionPoints[0].y,
                    ...
                    ngembryo.engine.eventLastPoint.push(dojo.connect(circ.getNode(), "onclick", function(event){
                         ...
                         * pass the coordinates of the first point instead.
                         */
                        ngembryo.engine.measure(grpRegion, ngembryo.engine.regionPoints[0].x, ngembryo.engine.regionPoints[0].y, ngembryo.engine.tempRegionPoints[0].x, ngembryo.engine.tempRegionPoints[0].y);
                        dojo.forEach(ngembryo.engine.eventLastPoint, dojo.disconnect);
                    }));
                    ngembryo.engine.eventLastPoint.push(dojo.connect(circ.getNode(), "onmouseenter", function(event){
                        circ.setFill([255, 255, 255, 1]);
