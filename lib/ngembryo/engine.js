...
var AnnotationEngine = new Class({
    initialize: function(target){
        ...
        this.eventCanvas2DMarker = []; // List of event handlers for the create2Dmarker canvas.
        this.eventLastPoint = []; // Event handler for polyline completion end point.
        this.eventCanvas2DRegion = []; // List of event handlers for the create2Dregion canvas.
    ...
    start: function(){
        if ($defined(this.target)) {
            ...
            this.iconManager.registerIcon('bookmark', new Icon('resources/images/Marker2D.png', 16, 16, 'cc'));
    ...
    hideCreateCanvas: function(){
        this.cc = false;
        dojo.forEach(this.eventCanvas2DRegion, dojo.disconnect);
        dojo.forEach(this.eventCanvas2DMarker, dojo.disconnect);
    },
    ...
    __update2DRegions: function(layerManager, data){
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            /* Scaling adjustment. */
            var sa = woolz.model.scl.cur / data[i].scale;
    ...
    activateCreateCanvas2DMarker: function(){
        if ($defined(this.surface)) {
            ...
            this.eventCanvas2DMarker.push(dojo.connect(cc.getNode(), "onclick", function(event){
    ...
    activateCreateCanvas2DRegion: function(){
        if ($defined(this.surface)) {
            ...
            this.eventCanvas2DRegion.push(dojo.connect(cc.getNode(), "onclick", function(event){
                var x = event.clientX - ngembryo.engine.dim.x;
                var y = event.clientY - ngembryo.engine.dim.y;
            ...
            this.eventCanvas2DRegion.push(dojo.connect(cc.getNode(), "onmousemove", function(event){
                if (ngembryo.engine.regionPoints.length > 0) {
                    ...
                    if (ngembryo.engine.regionPoints.length > 0) {
                        dojo.forEach(this.eventLastPoint, dojo.disconnect);
                    }
                    grpRegion.clear();
