...
function find_orientation($u, $mid) {
	global $con;
	$sql = "SELECT * FROM orientation WHERE deleted_at IS NULL AND owner='$u' AND model_id=$mid";
...
/* Encode orientation. */
function encode_orientation($o) {
	return '{id:'.$o['id'].',t:'.json_encode($o['title']).',d:'.json_encode($o['description']).',y:'.$o['yaw'].',p:'.$o['pitch'].',r:'.$o['roll'].',ds:'.$o['distance'].'}';
}
